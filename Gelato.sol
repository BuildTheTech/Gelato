...
contract DividendDistributor is IDividendDistributor {
    ...
    function depositForHex(uint256 _amount) external onlyToken {
        ...
        uint256 wplsAmount = WPLS.balanceOf(address(this)).sub(wplsBalanceBefore);
        ...
        uint256 hexAmount = HEX.balanceOf(address(this)).sub(hexBalanceBefore);

        totalHexDividends = totalHexDividends.add(hexAmount);
    ...
    function depositForStacked(uint256 _amount) external onlyToken {
        ...
        uint256 stackedAmount = STACKED.balanceOf(address(this)).sub(stackedBalanceBefore);

        totalStackedDividends = totalStackedDividends.add(stackedAmount);
    ...
    function getUnpaidSolidXEarnings(address shareholder) public view returns (uint256) {
        ...
        uint256 shareholderTotalDividends = getCumulativeSolidXDividends(shares[shareholder].amount);
        uint256 shareholderTotalExcluded = shares[shareholder].solidXTotalExcluded;
...
contract Gelato is IERC20, MultiAuth {
    ...
    function swapBack() internal swapping {
        ...
        uint256 balanceBefore = IERC20(SOLIDX).balanceOf(address(this));

        try pulseRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            amountToSwap,
            0,
            path,
            address(this),
            ...
            if(amountToLiquify > 0){
                try pulseRouter.addLiquidityETH{ value: amountSolidXLiquidity }(
                    ...
                    emit AutoLiquify(amountToLiquify, amountSolidXLiquidity);
                } catch {
    ...
    function setFees(uint256 _solidXBurnFee, uint256 _solidXReflectFee, uint256 _hexReflectFee, uint256 _stackedReflectFee, uint256 _liquidityFee, uint256 _feeDenominator, uint256 _totalSellFee) external authorizedFor(Permission.AdjustContractVariables) {    
        ...
        stackedReflectFee = _stackedReflectFee;
        liquidityFee = _liquidityFee;
        totalBuyFee = _liquidityFee.add(_solidXBurnFee).add(_solidXReflectFee).add(_hexReflectFee).add(_stackedReflectFee);
